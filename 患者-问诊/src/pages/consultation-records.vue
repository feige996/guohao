...
function getDispatchModeText(record: Appointment): string {
  if (record.consultMode === ConsultMode.DISPATCH) {
    if (record.assignMode === AssignMode.SMART) return '智能派单';
    if (record.assignMode === AssignMode.ASSIGN) return '指定医生';
  }
  if (record.consultMode === ConsultMode.CLINIC) return '在线坐诊';
  return '';
}
...
function getConsultationTypeText(record: Appointment): string {
  if (record.consultMode === ConsultMode.CLINIC) {
    if (record.consultType === ConsultType.VIDEO) return '视频问诊';
    if (record.consultType === ConsultType.VOICE) return '语音问诊';
  }
  return '图文问诊';
}
...
function getPrimaryButtonText(record: Appointment): string {
  ...
  // 1xx: 支付相关
  if (status === AppointmentStatus.PENDING_PAYMENT) return '去支付';
  if (status === AppointmentStatus.PAYMENT_FAILED) return '重新支付';
  
  // 303: 叫号中 - 最紧急的操作
  if (status === AppointmentStatus.CALLING) return '立即进入';
  
  // 302: 排队中
  if (status === AppointmentStatus.QUEUING) return '查看排队';
  
  // 305: 过号重排
  if (status === AppointmentStatus.RECALL_QUEUING) return '查看排队';
  
  // 201: 待接单
  if (status === AppointmentStatus.WAITING_ACCEPT) return '查看医生';
  
  // 301: 预约待生效
  if (status === AppointmentStatus.APPOINTMENT_PENDING) return '设置提醒';
  
  // 401: 进行中 - 继续问诊（进入IM）
  if (status === AppointmentStatus.CONSULTING) return '继续问诊';
  
  // 900: 已完成 - 查看记录（进入IM只读）
  if (status === AppointmentStatus.COMPLETED) return '查看记录';
  ...
  // 202: 派单失败
  if (status === AppointmentStatus.DISPATCH_FAILED) return '重新下单';
  
  // 503: 退款失败
  if (status === AppointmentStatus.REFUND_FAILED) return '联系客服';
...
<template>
  <view class="w-full min-h-screen bg-gray-100">
    ...
    <view class="p-24rpx">
      ...
      <view v-else class="flex flex-col gap-16rpx">
        <view
          ...
          :key="record.id"
          class="bg-white rounded-16rpx overflow-hidden transition-all active:opacity-90"
        ...
        >
          ...
          <view class="px-24rpx py-20rpx">
            ...
            <view class="flex items-center gap-20rpx mb-20rpx">
              ...
              />
              <view class="flex-1">
                <text class="block text-32rpx font-bold text-gray-900 mb-4rpx">
            ...
            <!-- 症状描述 -->
            <view v-if="record.symptoms" class="mb-20rpx px-20rpx py-16rpx bg-gray-50 rounded-12rpx">
            ...
            <view class="flex items-center justify-between">
              ...
              <view class="flex items-center gap-8rpx text-22rpx text-gray-400">
                <text>💰</text>
                <text class="font-bold text-primary">¥{{ record.actualAmount }}</text>
              </view>
            </view>
          </view>

          <!-- 操作按钮区 -->
          <view class="px-24rpx pb-20rpx flex gap-12rpx" @tap.stop>
            <!-- 主操作按钮 -->
            <wd-button
