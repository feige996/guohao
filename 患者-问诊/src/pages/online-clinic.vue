...
<template>
  <view class="w-full h-screen flex flex-col bg-gradient-to-b from-[#fffcfa] to-[#fff9f3]">
    <!-- 页头 -->
    <PageHeader title="在线坐诊" @back="handleBack" />

    <!-- Tab切换栏 -->
    <view class="flex-shrink-0 flex bg-white border-b border-gray-200 px-24rpx">
      <view
        v-for="tab in tabOptions"
        :key="tab.key"
        class="flex-1 relative flex items-center justify-center py-24rpx cursor-pointer transition-all"
      ...
      >
        ...
        <view v-if="activeTab === tab.key" class="absolute bottom-0 left-1/2 -translate-x-1/2 w-60rpx h-4rpx bg-gradient-to-r from-[#8e4337] to-[#6e2f25] rounded-2rpx" />
      </view>
    </view>
    ...
    <scroll-view scroll-y class="flex-1 min-h-0">
      <view class="px-24rpx py-24rpx flex flex-col gap-24rpx">
        <view
          v-for="doctor in filteredDoctors"
          :key="doctor.id"
          class="bg-gradient-to-br from-white to-[#fffcfa] rounded-18rpx px-32rpx py-32rpx shadow-sm border border-[#f5ebe9] overflow-hidden transition-all active:scale-98 active:shadow-xs"
        >
          <!-- 医生基本信息 -->
          <view class="flex gap-24rpx mb-24rpx">
            <view class="relative flex-shrink-0">
              <image
                :src="doctor.avatar"
                class="w-100rpx h-100rpx rounded-16rpx border-2 border-[#8e4337]/10"
                mode="aspectFill"
              />
              <view
                class="absolute -bottom-4rpx left-1/2 -translate-x-1/2 px-10rpx py-4rpx rounded-10rpx border border-white shadow-sm flex items-center justify-center"
                :class="doctor.status === 'consulting' ? 'bg-gradient-to-br from-red-500 to-red-600' : 'bg-gradient-to-br from-green-500 to-green-600'"
              >
                <text class="text-20rpx text-white font-semibold leading-none whitespace-nowrap">
            ...
            <view class="flex-1 flex flex-col gap-8rpx">
              <view class="flex items-center justify-between gap-16rpx">
                <text class="flex-1 text-30rpx font-bold text-gray-800">{{ doctor.name }}</text>
                <view class="flex items-center gap-16rpx flex-shrink-0">
                  <view class="flex items-center gap-6rpx">
                    <svg viewBox="0 0 16 16" fill="currentColor" class="w-20rpx h-20rpx text-amber-500">
                      <path d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z"/>
                    </svg>
                    <text class="text-24rpx text-amber-500 font-semibold">{{ doctor.rating }}</text>
                  </view>
                  <view class="flex items-center gap-4rpx px-12rpx py-6rpx bg-gradient-to-br from-[#f7ebe7] to-[#fdf7f5] border border-[#8e4337]/16 rounded-16rpx transition-all active:scale-95 active:bg-[#8e4337]/10" @tap="handleViewDoctorDetail(doctor)">
                    <text class="text-22rpx text-[#8e4337] font-medium">详情</text>
                    <svg viewBox="0 0 16 16" fill="currentColor" class="w-16rpx h-16rpx text-[#8e4337]">
          ...
          <!-- 状态信息 -->
          <view class="flex flex-col gap-12rpx mb-24rpx px-20rpx py-20rpx bg-gradient-to-br from-gray-50 to-gray-100 rounded-16rpx border border-gray-200">
            <view class="flex items-center gap-8rpx">
              <text class="text-24rpx text-gray-500 font-medium min-w-80rpx">状态：</text>
              <text 
            ...
            <view class="flex items-center gap-8rpx">
              <text class="text-24rpx text-gray-500 font-medium min-w-80rpx">费用：</text>
          ...
          <!-- 操作按钮 -->
          <button
            class="w-full h-80rpx border-none rounded-36rpx text-28rpx font-semibold transition-all flex items-center justify-center"
