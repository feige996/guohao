...
import { consultationRecords as mockConsultationRecords } from '@/data'
import { mockDoctors } from '@/data'
import type { Appointment } from '@/types'
import { AppointmentStatus, ConsultMode, AssignMode, ConsultType } from '@/types'
...
function getQueueStatusText() {
  if (!record.value || !record.value.queueNumber) return '等待中';
...
function getEstimatedWaitTime() {
  if (!record.value || !record.value.queueNumber) return '计算中...';
  const queueNumber = record.value.queueNumber;
  if (queueNumber === 1) {
    return '即将开始';
  }
...
function getDispatchModeText() {
  ...
  if (record.value.consultMode === ConsultMode.DISPATCH) {
    if (record.value.assignMode === AssignMode.SMART) return '智能派单';
    if (record.value.assignMode === AssignMode.ASSIGN) return '指定医生';
  }
  if (record.value.consultMode === ConsultMode.CLINIC) return '在线坐诊';
  return '';
}
...
function getConsultationTypeText() {
  ...
  if (record.value.consultMode === ConsultMode.CLINIC) {
    if (record.value.consultType === ConsultType.VIDEO) return '视频问诊';
    if (record.value.consultType === ConsultType.VOICE) return '语音问诊';
  }
  return '图文问诊';
}
...
function getStatusText() {
  ...
  const statusTextMap: Record<number, string> = {
    100: '等待支付', 101: '支付处理中', 102: '支付失败',
    200: '正在派单', 201: '等待医生接单', 202: '派单失败',
    300: '正在取号', 301: '预约成功', 302: '排队中', 303: '医生呼叫中', 304: '叫号失败', 305: '重新排队中',
    400: '待接诊', 401: '问诊进行中', 402: '服务即将结束',
    500: '退款申请中', 501: '退款审核中', 502: '退款处理中', 503: '退款失败',
    900: '已完成', 901: '已退款', 902: '已取消'
  };
...
<template>
  <view class="w-full h-screen bg-gray-50 flex flex-col">
    ...
    <view class="flex-shrink-0 flex items-center justify-between px-32rpx pt-24rpx pb-20rpx bg-white shadow-sm">
      ...
      <text class="flex-1 text-center text-34rpx font-bold text-gray-900">问诊详情</text>
      <view class="w-68rpx" />
    </view>
    ...
    <scroll-view v-if="record" class="flex-1" scroll-y>
      <view class="p-24rpx flex flex-col gap-16rpx">
        <!-- 状态卡片 -->
        <view class="bg-white rounded-16rpx overflow-hidden border-l-8rpx" 
          ...
          <view class="px-24rpx py-16rpx bg-gray-50 flex items-center justify-between">
            <view class="flex items-center gap-12rpx">
              <!-- 问诊模式标签 -->
              <view class="px-12rpx py-4rpx rounded-8rpx text-20rpx font-medium" 
              ...
              <!-- 排队号 -->
              <view v-if="record.queueNumber" class="px-12rpx py-4rpx rounded-8rpx bg-orange-500 text-white text-20rpx font-bold">
                {{ record.queueNumber }}号
              </view>
            </view>
            <!-- 状态标签 -->
            <view class="px-16rpx py-6rpx rounded-8rpx text-22rpx font-bold"
        ...
        <!-- 医生信息卡片 -->
        <view v-if="!isSmartWaiting()" class="bg-white rounded-16rpx p-24rpx">
          <view class="text-28rpx font-bold text-gray-900 mb-20rpx">医生信息</view>
          <view class="flex items-center gap-20rpx">
            <image :src="getDoctorInfo().avatar" class="w-96rpx h-96rpx rounded-full border-4rpx border-gray-100" mode="aspectFill" />
            <view class="flex-1 flex flex-col gap-8rpx">
              <text class="text-32rpx font-bold text-gray-900">{{ getDoctorInfo().name }}</text>
        ...
        <view class="bg-white rounded-16rpx p-24rpx">
          <view class="text-28rpx font-bold text-gray-900 mb-20rpx">订单信息</view>
          <view class="flex flex-col gap-16rpx">
            <view class="flex items-center justify-between py-12rpx border-b border-gray-100">
            ...
            <view class="flex items-center justify-between py-12rpx border-b border-gray-100">
              ...
              <text class="text-28rpx font-bold text-primary">¥{{ record.actualAmount }}</text>
            </view>
        ...
        <!-- 处方状态卡片 -->
        <view v-if="record.prescriptionId || (record.status === AppointmentStatus.CONSULTING)" class="bg-white rounded-16rpx p-24rpx">
          <view class="text-28rpx font-bold text-gray-900 mb-20rpx">处方状态</view>
          <view class="px-20rpx py-16rpx rounded-12rpx" 
      ...
      <view class="p-24rpx flex flex-col gap-16rpx">
        <!-- 核心操作区 -->
        <view class="text-24rpx font-bold text-gray-700 mb-8rpx">核心操作</view>
        ...
        <!-- 辅助功能区 -->
        <view class="mt-16rpx">
          <view class="text-24rpx font-bold text-gray-700 mb-8rpx">辅助功能</view>
        ...
        <!-- 信息查看区 -->
        <view class="mt-16rpx">
          <view class="text-24rpx font-bold text-gray-700 mb-8rpx">信息查看</view>
          <view class="flex flex-col gap-8rpx">
            <view v-if="record.doctorId" class="flex items-center justify-between py-12rpx px-16rpx bg-gray-50 rounded-12rpx active:bg-gray-100" @tap="() => uni.showToast({ title: '查看医生详情', icon: 'none' })">
              <text class="text-24rpx text-gray-700">查看医生详细信息</text>
              <text class="text-24rpx text-gray-400">></text>
            </view>
            <view class="flex items-center justify-between py-12rpx px-16rpx bg-gray-50 rounded-12rpx active:bg-gray-100" @tap="() => uni.showToast({ title: '查看订单详情', icon: 'none' })">
              <text class="text-24rpx text-gray-700">查看订单详细信息</text>
              <text class="text-24rpx text-gray-400">></text>
            </view>
            <view v-if="record.prescriptionId" class="flex items-center justify-between py-12rpx px-16rpx bg-gray-50 rounded-12rpx active:bg-gray-100" @tap="() => uni.showToast({ title: '查看处方详情', icon: 'none' })">
              <text class="text-24rpx text-gray-700">查看处方详情</text>
              <text class="text-24rpx text-gray-400">></text>
            </view>
            <view v-if="record.status === AppointmentStatus.COMPLETED" class="flex items-center justify-between py-12rpx px-16rpx bg-gray-50 rounded-12rpx active:bg-gray-100" @tap="() => uni.showToast({ title: '查看完整病历', icon: 'none' })">
    ...
    <!-- 排队状态弹窗 -->
    <view v-if="showQueueModal && record" class="fixed inset-0 bg-black/50 z-200 flex items-center justify-center p-32rpx" @tap="closeQueueModal">
      <view class="w-full max-w-600rpx bg-white rounded-24rpx flex flex-col overflow-hidden" @tap.stop>
        <view class="flex items-center justify-between px-32rpx py-24rpx border-b border-gray-200">
          <text class="text-32rpx font-bold text-gray-800">排队状态</text>
          <button class="w-56rpx h-56rpx flex items-center justify-center rounded-full bg-gray-100 border-none transition-all active:bg-gray-200" @tap="closeQueueModal">
        ...
        <view class="p-32rpx flex flex-col gap-32rpx">
          ...
          <view class="flex flex-col items-center gap-16rpx">
            ...
            <view class="flex items-baseline gap-8rpx">
              <text class="text-96rpx font-bold text-primary">{{ record.queueNumber || 0 }}</text>
              <text class="text-36rpx text-gray-500">号</text>
            </view>
            <view class="px-24rpx py-12rpx rounded-full text-24rpx font-semibold" :class="record.queueNumber === 1 ? 'bg-red-100 text-red-600' : 'bg-blue-100 text-blue-600'">
              {{ getQueueStatusText() }}
            </view>
          </view>

          <!-- 医生信息 -->
          <view class="flex items-center gap-20rpx p-20rpx rounded-16rpx bg-gray-50">
            <image :src="getDoctorInfo().avatar" class="w-80rpx h-80rpx rounded-full" mode="aspectFill" />
            <view class="flex-1 flex flex-col gap-8rpx">
              <text class="text-28rpx font-bold text-gray-800">{{ getDoctorInfo().name }}</text>
              <text class="text-24rpx text-gray-600">{{ getDoctorInfo().title }}</text>
            </view>
          </view>
          ...
          <!-- 等待信息 -->
          <view class="flex flex-col gap-16rpx">
            <view class="flex items-center gap-16rpx p-20rpx rounded-12rpx bg-blue-50">
              <text class="text-32rpx">⏰</text>
              <view class="flex-1 flex flex-col gap-4rpx">
                <text class="text-22rpx text-gray-500">预计等待</text>
                <text class="text-26rpx font-bold text-blue-600">{{ getEstimatedWaitTime() }}</text>
              </view>
            </view>
            <view class="flex items-center gap-16rpx p-20rpx rounded-12rpx bg-green-50">
              <text class="text-32rpx">📋</text>
              <view class="flex-1 flex flex-col gap-4rpx">
                <text class="text-22rpx text-gray-500">问诊方式</text>
                <text class="text-26rpx font-bold text-green-600">{{ getConsultationTypeText() }}</text>
              </view>
            </view>
          </view>

          <!-- 温馨提示 -->
          <view class="p-20rpx rounded-12rpx bg-yellow-50 border border-yellow-200 flex flex-col gap-12rpx">
            <text class="text-24rpx font-semibold text-yellow-700">💡 温馨提示</text>
