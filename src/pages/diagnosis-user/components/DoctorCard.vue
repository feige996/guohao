  ...
  >
    ...
    <view class="relative flex-shrink-0">
      <image
        :src="doctor.avatar"
        :class="avatarSize"
        class="border-2rpx border-primary/10 rounded-16rpx"
        mode="aspectFill"
      />
      <!-- 在线状态 -->
      <view
    ...
    <view class="flex flex-1 flex-col gap-12rpx">
      <view class="flex items-center gap-16rpx">
        <text class="text-30rpx text-gray-800 font-bold">{{ doctor.name }}</text>
        <view
          v-if="showOnlineStatus && doctor.isOnline"
          class="flex items-center gap-8rpx rounded-full bg-green-500/12 px-12rpx py-4rpx"
        >
      ...
      <!-- 统计信息 -->
      <view v-if="showStats" class="mt-6rpx flex items-center gap-16rpx">
