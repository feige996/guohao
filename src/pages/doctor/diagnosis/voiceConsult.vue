...
<template>
  <view class="min-h-screen flex flex-col bg-gray-800 text-white">
    <!-- 顶部导航 -->
    <view class="sticky top-0 z-10 flex items-center justify-between bg-gray-800 p-4 border-b border-gray-700">
    ...
    <view class="flex flex-col items-center justify-center p-8">
      <image :src="patientInfo.avatar" class="mb-4 h-24 w-24 rounded-full border-4 border-blue-500" />
    ...
    <!-- 消息区域 -->
    <view class="flex-1 p-4 overflow-y-auto message-list">
      <view v-for="message in messages" :key="message.id" class="mb-4">
        ...
        <view v-if="message.sender === 'doctor'" class="flex items-end gap-2">
          ...
          <view>
            <view class="max-w-[70%] rounded-tl-none rounded-tr-xl rounded-bl-xl rounded-br-xl bg-blue-600 p-3">
        ...
        <view v-else class="flex items-end justify-end gap-2">
          <view class="text-right">
            <view class="max-w-[70%] rounded-tl-xl rounded-tr-xl rounded-bl-xl rounded-br-none bg-gray-700 p-3">
              <text>{{ message.content }}</text>
            </view>
            <text class="mt-1 text-xs text-gray-400">{{ message.time }}</text>
          </view>
    ...
    <!-- 通话控制按钮 -->
    <view class="flex justify-around items-center py-6">
      <!-- 静音按钮 -->
      <view class="flex flex-col items-center">
        <view class="flex items-center justify-center h-14 w-14 rounded-full bg-gray-700" @click="toggleMute">
      ...
      <!-- 挂断按钮 -->
      <view class="flex flex-col items-center">
        <view class="flex items-center justify-center h-16 w-16 rounded-full bg-red-500" @click="handleHangup">
      ...
      <!-- 免提按钮 -->
      <view class="flex flex-col items-center">
        <view class="flex items-center justify-center h-14 w-14 rounded-full bg-gray-700" @click="toggleSpeaker">
          <text class="text-2xl">🔊</text>
        </view>
