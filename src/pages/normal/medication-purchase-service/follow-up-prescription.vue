...
<template>
  <div class="min-h-screen bg-[#f5f5f5]">
    ...
    <!-- 患者信息卡片 -->
    <div class="bg-white m-[16px] rounded-[8px] p-[16px] shadow-sm">
      <div class="flex justify-between items-center mb-[16px]">
        <h2 class="text-[16px] font-medium text-[#333333]">患者信息</h2>
    ...
    <!-- 医嘱列表 -->
    <div class="bg-white m-[16px] rounded-[8px] p-[16px] shadow-sm">
      <h2 class="text-[16px] font-medium text-[#333333] mb-[16px]">医嘱列表</h2>
      
      <!-- 医嘱项 -->
      <div v-for="item in prescriptionItems" :key="item.id" class="flex justify-between items-center py-[12px] border-b border-[#f0f0f0] last:border-b-0">
        <div class="flex-1">
          <div class="text-[14px] font-medium text-[#333333] mb-[4px]">{{ item.name }}</div>
          <div class="text-[12px] text-[#999999]">{{ item.frequency }} · {{ item.duration }}</div>
        </div>
    ...
    <div class="mx-[16px] mb-[24px]">
      <p class="text-[12px] text-[#999999] line-clamp-2">
        勾选您想继续使用的医嘱后，提交即可申请。
      </p>
    </div>

    <!-- 底部按钮 -->
    <div class="fixed bottom-0 left-0 right-0 bg-white p-[16px] border-t border-[#e5e5e5]">
      <div class="flex gap-[12px]">
        <wd-button 
          type="default" 
          class="flex-1 h-[44px] rounded-[22px] text-[16px]"
