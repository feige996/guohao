...
<template>
  <div class="relative min-h-screen w-full bg-[#F9FAFB] pb-24">
    ...
    <!-- 主内容区 -->
    <div class="flex flex-col gap-5 px-4 pb-4 pt-[56px] box-border">
      <!-- 问诊方式设置 -->
      <section>
        ...
        <!-- 图文+语音消息 -->
        <div class="mb-4 w-full rounded-[20px] bg-white p-5 shadow-[0px_4px_12px_0px_rgba(0,0,0,0.05)] box-border">
          <div class="mb-4 flex items-center justify-between">
            <div class="flex items-center gap-3">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="#8E4337" class="h-5 w-5">
                <path d="M20 2H4c-1.1 0-2 .9-2 2v18l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm0 14H6l-2 2V4h16v12z" />
              </svg>
              <span class="text-[#333333] font-medium truncate">图文+语音消息</span>
            </div>
            <button
              class="relative h-6 w-12 cursor-pointer rounded-full transition-colors"
            ...
            >
              <div
                class="absolute top-0.5 h-5 w-5 border rounded-full bg-white shadow-sm transition-transform"
                :class="consultationTypes.text ? 'border-[#52c41a] translate-x-[26px]' : 'border-[#D1D5DB] translate-x-0.5'"
              />
            </button>
          </div>
            ...
            <div class="border-t border-[#E5E7EB] pt-4">
              ...
              <div class="relative w-full rounded-lg overflow-hidden">
                ...
                <input
                  ...
                  class="w-full h-full border-none bg-[#F5EBE9] py-3 pl-8 pr-4 text-lg font-bold focus:ring-2 focus:ring-[#8E4337] box-border"
        ...
        <div class="mb-4 w-full rounded-[20px] bg-white p-5 shadow-[0px_4px_12px_0px_rgba(0,0,0,0.05)] box-border">
          <div class="mb-4 flex items-center justify-between">
            <div class="flex items-center gap-3">
              ...
              <span class="text-[#333333] font-medium truncate">实时视频问诊</span>
            </div>
            <button
              class="relative h-6 w-12 cursor-pointer rounded-full transition-colors"
            ...
            >
              <div
                class="absolute top-0.5 h-5 w-5 border rounded-full bg-white shadow-sm transition-transform"
                :class="consultationTypes.video ? 'border-[#52c41a] translate-x-[26px]' : 'border-[#D1D5DB] translate-x-0.5'"
              />
            </button>
          </div>
        ...
        <!-- 实时语音问诊 -->
        <div class="w-full rounded-[20px] bg-white p-5 shadow-[0px_4px_12px_0px_rgba(0,0,0,0.05)] box-border">
          <div class="mb-4 flex items-center justify-between">
            <div class="flex items-center gap-3">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="#8E4337" class="h-5 w-5">
                <path d="M20.01 15.38c-1.23 0-2.42-.2-3.53-.56-.35-.12-.74-.03-1.01.24l-1.57 1.97c-2.83-1.35-5.48-3.9-6.89-6.83l1.95-1.66c.27-.28.35-.67.24-1.02-.37-1.11-.56-2.3-.56-3.53 0-.54-.45-.99-.99-.99H4.19C3.65 3 3 3.24 3 3.99 3 13.28 10.73 21 20.01 21c.71 0 .99-.63.99-1.18v-3.45c0-.54-.45-.99-.99-.99z" />
              </svg>
              <span class="text-[#333333] font-medium truncate">实时语音问诊</span>
            </div>
            <button
              class="relative h-6 w-12 cursor-pointer rounded-full transition-colors"
            ...
            >
              <div
                class="absolute top-0.5 h-5 w-5 border rounded-full bg-white shadow-sm transition-transform"
                :class="consultationTypes.voice ? 'border-[#52c41a] translate-x-[26px]' : 'border-[#D1D5DB] translate-x-0.5'"
              />
            </button>
          </div>
            ...
            <div>
              ...
              <div class="relative w-full rounded-lg overflow-hidden">
                <input
                  v-model="duration"
                  type="number"
                  class="w-full h-full border-none bg-[#F5EBE9] py-3 pl-4 pr-16 text-lg text-[#333333] font-bold focus:ring-2 focus:ring-[#8E4337] box-border"
                  aria-label="计划时长"
                >
    ...
    <!-- 底部按钮 -->
    <div class="fixed bottom-0 left-0 right-0 z-50 w-full border-t border-[#E5E7EB] bg-white/95 p-4 backdrop-blur-sm box-border" style="max-width: 100vw; overflow-x: hidden;">
      <div class="flex gap-3">
        ...
        <button
          class="flex flex-1 cursor-pointer items-center justify-center gap-2 rounded bg-[#8E4337] py-3 text-sm text-white font-medium leading-5 transition-colors active:scale-98 hover:bg-[#6E2F25] focus:outline-none focus:ring-2 focus:ring-[#8E4337]"
          :disabled="isLoading"
          :class="isLoading && 'relative text-transparent'"
          aria-label="确认开启坐诊"
          @click="handleStartConsultation"
        >
          <svg v-if="!isLoading" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="h-4 w-4">
            <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.857-9.809a.75.75 0 00-1.214-.882l-3.483 4.79-1.88-1.88a.75.75 0 10-1.06 1.061l2.5 2.5a.75.75 0 001.137-.089l4-5.5z" clip-rule="evenodd" />
          </svg>
          <span v-if="!isLoading">开启坐诊</span>
